...
import logging
from models.factorioModels import Player, server, mod_database, Mod, ServerConfig
...
async def handle_players(ctx, player_data: str):
    ...
    for player_line in player_data.split('\n')[1:]:
        ...
        table.addRow(p.username, str(p.is_online))
...
#        extras = [
#            Config.cfg.rcon.mods_directory / 'mod-list.json',
#            Config.cfg.rcon.mods_directory / 'mod-settings.json',
#        ]
#        for extra in extras:
#            if extra.exists():
#                tmp_zip.write(extra, arcname=Path('mods') / extra.name)

#    all_mods_file = all_mods_dir /
#    'all-mods-{}.zip'.format(datetime.now().strftime('%Y-%m-%d_%H-%m-%S'))
...
def handle_ip(ip):
    if not Config.cfg.rcon.host:
        Config.cfg.rcon.host = ip
        logger.info('Server IP set: {}'.format(Config.cfg.rcon.host))